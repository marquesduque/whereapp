...
function fillInAddress() {
    ...
    $("#horario_lat").val(lat.toString().replace('.', ','));
    $("#horario_lng").val(lng.toString().replace('.', ','));

    $(place.address_components).each(function (index, item) {
        $(item.types).each(function (indext, itemt) {
            switch (itemt) {
                case "street_number":
                    $("#horario_numero").val(item.long_name);
                    break;

                case "route":
                    $("#horario_endereco").val(item.long_name);
                    break;
                ...
                case "administrative_area_level_2":
                    $("#horario_cidade").val(item.long_name);
                    break;

                case "country":
                    $("#horario_pais").val(item.long_name);
                    break;

                case "postal_code":
                    $("#horario_cep").val(item.long_name);
                    break;
            }
        })
    });
