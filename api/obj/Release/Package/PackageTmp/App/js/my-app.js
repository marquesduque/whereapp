...
window.onmessage = function (e) {
    ...
    localStorage.setItem('pushToken', dados.data.pushToken);
    localStorage.setItem('plataforma_id', dados.data.plataforma);
};
...
function onloadpage(callback) {
    ...
    creait.get('filiais', null, function (data) {
        debugger;
        if (controller.filial.get == null) {
            window.location = '/';
            return;
        }
        if (controller.filial.get() == null || data.filiais.filter(function (a, b) { return a.id == controller.filial.get().id && a.versao == controller.filial.get().versao }).length == 0) {
            if (data.error == null) {
                if (controller.filial.get() != null && controller.filial.get().length != 0) {
                    controller.filial.set(data.filiais.filter(function (a, b) { return a.id == controller.filial.get().id && a.versao != controller.filial.get().versao }));
                }
                else if (data.filiais.length == 1){
                    controller.filial.set(data.filiais[0]);
                }
                controller.busca.set(null);
                ...
                myApp.alert("Estamos em manutenção, tente acessar o aplicativo mais tarde!", "Oops");
            }
        }
        else {
            bind(controller.layout.get());
            callback();
        }
    }, false);
