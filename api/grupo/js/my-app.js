            ...
            try {
                ...
                if (method in window["controller"]) {
                    ...
                    var redirect = eval("controller." + method + '.init')(function (clone, callback) {
                        ...
                        if (controller.loginE.get() != null) {
                            $('#nomeUsuario').html(controller.loginE.get().nome);
                            $('.email_user').html(controller.loginE.get().email);
...
function finalizar_compra() {
    ...
    compra.comprador_id = controller.loginE.get().id;

    if (controller.cliente.get() != null && controller.loginE.get().perfil_id == 3) {
        compra.vendedor_id = controller.cliente.get().id;
    }
    else if (controller.cliente.get() != null && controller.loginE.get().perfil_id == 5) {
        compra.comprador_id = controller.cliente.get().id;
        compra.vendedor_id = controller.loginE.get().id;
    }
    else if (controller.cliente.get() != null && controller.loginE.get().perfil_id == 6) {
        compra.comprador_id = controller.cliente.get().id;
        compra.vendedor_id = controller.loginE.get().id;
    }
    else if (controller.cliente.get() != null && controller.loginE.get().perfil_id == 7) {
        compra.comprador_id = controller.cliente.get().id;
        compra.vendedor_id = controller.loginE.get().id;
    }
    ...
    $(carrinho.produtos).each(function (index, item) {
        ...
        if (controller.cliente.get() != null && controller.loginE.get().perfil_id == 3) {
            pedido.horario.cliente_id = controller.loginE.get().id;
        }
        else if (controller.cliente.get() != null && controller.loginE.get().perfil_id == 5) {
    ...
    creait.post('pedido', compra, function (data_pedido) {
        ...
        if (data_pedido.erro != null) {
            ...
            if (data_pedido.pagarme_status == null || data_pedido.pagarme_status == "approved" || data_pedido.pagarme_status == "authorized") {
                ...
                //debugger;
                if (controller.loginE.get().perfil_id == 3) {
                    myApp.alert('', 'Pedido realizado com sucesso');
                    creait.redirect('categoria');
                }
                else if (controller.loginE.get().perfil_id == 5) {
                    creait.redirect('mesa', 'id=' + controller.cliente.get().cpf);
                }
                else if (controller.loginE.get().perfil_id == 7) {
