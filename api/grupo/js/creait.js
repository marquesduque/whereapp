...
var creait = {
    url: "http://192.168.0.6", //ambiente hom ===>http://onfarmhom.creait.com.br/ ambiente prod ===>http://onfarm.creait.com.br/
    init: function () {
    ...
    redirect: function (page, parameter) {
        mainView.router.loadPage("pagina/" + page + ".html" + (parameter == null ? '' : '?' + parameter));
    },
    ...
    preprocess: function (content, url, next) {
        ...
        if (method in window["controller"]) {
            eval("controller." + method + '.init')(function (data, callback) {
                ...
                var password = window.location.hostname;
                ...
                var key = '';
                try {
                    key = decrypt($('meta[name="creait"]').attr('content'), password);
                }
                catch (ex) { }

                if (key == window.location.hostname || window.location.hostname.toLowerCase().indexOf('creait') != -1) {
                    result = template(data);
                }
                else {
                    result = '<div class="pages"><div data-page="licenca" class="page" style="background-color: #5f126c; text-align: center;">LICENÇA CREAIT INVÁLIDA</div></div>';
                }
                next(result);
                if (callback != null) {
                    callback(result);
...
function readimg(e) {
    ...
    fileReader.onload = function (e) {
        ...
        img.onload = function () {
            ...
            $($(control).attr('source') + '_read').attr('src', dataUrl)
...
CryptoJS = CryptoJS || function (g, j) {
