...
function atualiza_compra(id, status) {

    $('.loader').show();

        $.ajax({
            type: "GET",
            url: '/compras/AtualizaCompra?id=' + id + '&status=' + status,
            dataType: 'text',
            contentType: false,
            processData: false,
            success: function (result) {
                if (result != "") {
                    ...
                    if (status == 2) {
                        ...
                        $('#btnPreparo').css('display', 'none');
                        $('#btnFinalizar').css('display', 'unset');
                        $('#preparo').removeClass('small-box bg-aqua').addClass('small-box bg-yellow');
                        $('#dataPreparo').text(result);
                        $("#compra_id").select2("val", status);
                        modal('Tudo certo, este pedido está sendo preparado para ser enviado.');

                    } else if (status == 3) {

                        $('#btnFinalizar').css('display', 'none');
                        $('#btnEntregue').css('display', 'unset');
                        $('#finalizado').removeClass('small-box bg-aqua').addClass('small-box bg-yellow');   
                        $('#dataFinalizado').text(result);
                        $("#compra_id").select2("val", status);
                        modal('Tudo certo, este pedido foi enviado para entrega.');

                    } else if (status == 4) {

                        $('#btnEntregue').css('display', 'none');
                        $('#cancelar').css('display', 'none');
                        $('#entregue').removeClass('small-box bg-aqua').addClass('small-box bg-yellow'); 
                        $('#dataEntregue').text(result);                        
                        $("#compra_id").select2("val", status);

                        $('#salvarEndereco').removeClass('btn btn-success btn-block').addClass('btn btn-success btn-block disabled');
                        ...
                        modal('Tudo certo, a entrega foi concluída com sucesso.');
                        ...
                        $('#btnEntregue').css('display', 'none');
                        $('#cancelar').css('display', 'none');
                        $('#entregue').css('display', 'none');
                        $('#cancelamento').css('display', 'block');
                        $('#dataCancelamento').text(result);   
                        $("#compra_id").select2("val", status);
                        ...
                        modal('Pronto, este pedido foi cancelado som sucesso.');
                    }
